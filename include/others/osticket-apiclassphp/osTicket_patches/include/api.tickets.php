    ...
    */
    ...
    function reply($format) {
        ...
        if(!strcasecmp($format, 'email')) {
            ...
            # Parse request body
            $ticket = $this->replyTicket($this->getRequest($format));
        }
	...
	$ticket = Ticket::getIdByNumber($data['ticketID']);
	$ticket=new Ticket($ticket);
	$sql="SELECT `user_id` FROM `".USER_EMAIL_TABLE."` WHERE `address`=".db_input($data['email'])."";
	if(!($res=db_query($sql)) ||  !db_num_rows($res)) {
		return false;
	} else {
		$data['userId']=db_fetch_array($res);
		$data['userId']=(int)$data['userId']['user_id'];
		if ($data['userId']==0) return false;
	}
	$sql="SELECT `ticket_id` FROM `".TICKET_TABLE."` WHERE `number`=".(int)$data['ticketID']." and `user_id`=".(int)$data['userId']."";	// almost the same as getIdByNumber
	if(!($res=db_query($sql)) ||  !db_num_rows($res)) {
		return false;
	} else {
	        $ticket = $ticket->postMessage($data, $data['source']);
