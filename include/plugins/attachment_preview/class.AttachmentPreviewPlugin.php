    ...
    function bootstrap() {
        ...
        Signal::connect(self::signal_id, function ($object, $data) {
            ...
            if (version_compare(PHP_VERSION, '5.4.0') >= 0)
                AttachmentPreviewPlugin::$instance->debug_log("Received connection from %s", get_class($object));
            // 
        ...
        if (self::isTicketsView() && $this->getConfig()->get('attachment-enabled')) {
            // This debug_log line only works in php5.4+             
            if (version_compare(PHP_VERSION, '5.4.0') >= 0)
                AttachmentPreviewPlugin::$instance->debug_log("Agent requested a tickets-view: Starting the attachments plugin.");
